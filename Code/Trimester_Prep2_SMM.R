...
df_wrong$LGW_7 <- as.Date(df_wrong$Date) - 7
df_wrong$LGW_6 <- as.Date(df_wrong$Date) - 6
df_wrong$LGW_5 <- as.Date(df_wrong$Date) - 5
df_wrong$LGW_4 <- as.Date(df_wrong$Date) - 4
df_wrong$LGW_3 <- as.Date(df_wrong$Date) - 3
df_wrong$LGW_2 <- as.Date(df_wrong$Date) - 2
df_wrong$LGW_1 <- as.Date(df_wrong$Date) - 1
df_wrong$LGW_0 <- as.Date(df_wrong$Date) 
df_wrong$Tr3_end <- df_wrong$Date

birth3 <- df_wrong %>%
  mutate(row = row_number())%>%
  mutate(zip=as.numeric(ptzip))%>%
  select(starts_with("LGW"), zip, row, Shepsid) %>%
  tidyr::pivot_longer(c(-"zip", -"row", -"Shepsid")) %>%
  group_by(zip)%>%
  mutate(Date=as.Date(value))%>%
  filter(month(Date) >= 10 | month(Date) <= 4)%>%
...
LGW_grp_cw <- birth3 %>%
  group_by(Shepsid)%>%
  summarize(LGW_cw_count=sum(coldwave))
...
LGW_grp_hw <- birth3 %>%
  group_by(Shepsid)%>%
  summarize(LGW_hw_count=sum(heatwave))
...
births_hw <- df_wrong %>%
  ...
  left_join(LGW_grp_cw, by=c('Shepsid'))%>%
  left_join(LGW_grp_hw, by=c('Shepsid'))%>%
  dplyr::select(Shepsid, SMM21, ptzip, Date, Tr1, Tr2, Tr3, Last_Gest_Week, Year, Age, Race, Insurance, Ethnicity,
                weeks_gest, Tr1_hw_count, Tr2_hw_count, Tr3_hw_count, Tr1_cw_count, Tr2_cw_count, Tr3_cw_count, LGW_hw_count, LGW_cw_count)

# Heatwaves during last week of gestation

write_parquet(births_hw, "Data/Trimester_Exposure_DELIVERIES.parquet")
