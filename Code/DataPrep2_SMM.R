...
write_parquet(tabdat, "Data/Deliveries_w_SMM.parquet")
...
catVars <- c("SMM21", "Year", "Age", "Race", "Ethnicity", "Insurance", "ICE_Income_Tertile", "ICE_Race_Tertile", "RUCA_Cat", "geo_region")
...
write.csv(smm_tabmat, file = "Results/TableOne_SMM_Deliveries.csv")
...
smm_tabmat <- print(smm_tab, showAllLevels = TRUE, exact = "stage", quote = FALSE, noSpaces = TRUE, printToggle = FALSE, smd = FALSE)
write.csv(smm_tabmat, file = "Results/TableOne_SMM_Warm_Season_Deliveries.csv")

tabcold <- tabdat %>%
  filter(month(Date) <= 4 | month(Date) >= 10)
smm_tab <- CreateTableOne(vars = catVars, data = tabcold, factorVars = catVars)
smm_tabmat <- print(smm_tab, showAllLevels = TRUE, exact = "stage", quote = FALSE, noSpaces = TRUE, printToggle = FALSE, smd = FALSE)
write.csv(smm_tabmat, file = "Results/TableOne_SMM_Cold_Season_Deliveries.csv")
...
write_parquet(deliv,"Data/Outcomes_SMM_v2_DELIVERIES.parquet")

#### Create shapefile of SMM counts for mapping ####

data <- read_parquet("Data/Outcomes_SMM_v2_DELIVERIES.parquet")%>%
  group_by(zip)%>%
...
write_sf(shp, "Data/shp/SMM_DELIVERIES.shp")
